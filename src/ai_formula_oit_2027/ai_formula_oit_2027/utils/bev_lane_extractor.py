...
class BEVLaneExtractor:
    ...
    def extract_lane_trajectories(
        ...
        """
        annotated_bev = cv2.cvtColor(bev_mask, cv2.COLOR_GRAY2BGR)
        ...
        if has_left and has_right:
            # Check if one side is significantly better/cleaner than the other
        ...
        # Draw Target Path onto BEV image (Bright Green line)
        if target_center_metric is not None and len(target_center_metric) >= 2:
            path_px = [self._metric_to_px(pt[0], pt[1]) for pt in target_center_metric]
            for i in range(len(path_px) - 1):
                ...
                if 0 <= p1[0] < self.bev_w and 0 <= p1[1] < self.bev_h and \
                   0 <= p2[0] < self.bev_w and 0 <= p2[1] < self.bev_h:
                    cv2.line(annotated_bev, p1, p2, (0, 255, 50), 3)
